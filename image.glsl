...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  FRACTALIFY(float, vec4, value, perlinNoiseUnsigned4, arg, lacunarity, gain, layers);
  FRACTALIFY(vec4, vec4, grad, perlinNoiseUnsignedGradient4, arg, lacunarity, gain, layers);
  /* FRACTALIFY(float, vec2, value, valueNoiseUnsigned2, arg, lacunarity, gain, layers); */
  /* FRACTALIFY(vec2, vec2, grad, valueNoiseUnsignedGradient2, arg, lacunarity, gain, layers); */
  ...
  float y = (fragCoord / iResolution.xy).y;
  float lineThickness = 0.01;
  grad *= (sin(iTime) / 2 + 0.5);
  fragColor.rgb = vec3(value);
  fragColor.rg += grad.xy;
  /* fragColor.rgb = (1 - vec3(step(value, y))) / 2.0; */
  /* fragColor.r += (abs(grad.y - y + 0.5) < lineThickness / 2.0) ? 1 : 0; */
